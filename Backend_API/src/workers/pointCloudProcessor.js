...
const SCRIPT_PATH = path.join(config.SCRIPTS_DIR, config.SCRIPT_NAME_POINTCLOUD);

async function processPointCloud(jobId, { projectId, filePath }) {
  await jobQueue.updateStatus(jobId, 'processing');
  const inputPath = fileStorage.getRawFilePath(projectId, filePath);
  const outputDir = await fileStorage.getProcessedPath(projectId, 'PointClouds');
  ...
  const lodRatio = process.env.LOD_POINTCLOUD_RATIO;
  const args = [SCRIPT_PATH, inputPath, '-o', outputDir];
  if (lodRatio != null && lodRatio !== '') {
    const r = parseFloat(lodRatio);
    if (!Number.isNaN(r) && r > 0 && r < 1) args.push('--lod-ratio', String(r));
  }
...
module.exports = { processPointCloud };
